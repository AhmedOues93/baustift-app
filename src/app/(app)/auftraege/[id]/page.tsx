...
export default async function AuftragPage({params}:{params:Promise<{id:string}>}) {
 ...
 const [{data:a},{data:kunden}]=await Promise.all([supabase.from("auftraege").select("*").eq("id",id).eq("user_id",user.id).maybeSingle(),supabase.from("kunden").select("*").eq("user_id",user.id)]);
 if(!a) notFound(); const k=((kunden??[]) as Kunde[]).find(k=>k.id===a.kunde_id);
 return <div className="flex flex-col gap-4"><Link href="/auftraege" className="text-sm font-medium text-text-leise">← Aufträge</Link><AuftragEditor auftrag={a as Auftrag} kunde={k??null}/></div>;
}